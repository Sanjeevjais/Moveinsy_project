...
const registerRide = async (req, res) => {
  ...
  try {
    ...
    const userDetails = await user_model.findById(req.user._id);
    const userUpdate = await user_model.findByIdAndUpdate(req.user._id, { ride_status: "RIDING",
      total_ride: [...userDetails.total_ride, ride_created[0]._id]
     }, { session });
...
const getAllRideDetails = async (req, res) => {
  // console.log(req.user._id.toString());
  try {
    const rides = await ride_model.find({
      "approved_passengers._id": req.user._id.toString(),
    });
    ...
    logger.error("Error while fetching all ride details: ", error);
    res.status(500).send({ error: "Failed to fetch all ride details" });
  }
}
