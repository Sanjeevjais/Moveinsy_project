...
const verifySignUpBody = async (req, res, next) => {
  ...
  // console.log("Verifying Signup Body:", req.body);
  try {
...
const verifySignInBody = (req, res, next) => {
  console.log("Verifying login Body:", req.body);
  try {
    if (!req.body.username || req.body.username.length < 3 || req.body.username.length > 20) {
      return res.status(401).send({
        error: "Username size should be 3 to 20 characters",
...
const authenticate = (req, res, next) => {
  try {
    ...
    if (!token) {
      return next(); // no user, but not an error here
    }
...
/**
 * Optional auth: attaches user if token valid, otherwise continues as anonymous.
 */
